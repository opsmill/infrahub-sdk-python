...
"""Client-side failures (connection error, timeout) that are always considered transient."""

CONNECTION_LOST_EXCEPTIONS = (httpx.NetworkError, httpx.ConnectTimeout, httpx.RemoteProtocolError)
"""httpx failures meaning the connection was lost or never established, mapped to ``ServerNotReachableError``.

``ConnectTimeout`` is included because a load balancer that drops packets during a failover leaves the TCP
handshake hanging instead of refusing it, and httpx classifies it as a timeout rather than a network error.
...
shape a failover takes on a request that was already in flight. The other timeouts (read, write while sending
a request body, connection pool) are mapped to ``ServerNotResponsiveError`` by the client.
"""
...
class TransientRetryHandler:
    ...
    A failure is transient when it is a connection error, a timeout, an HTTP response whose
