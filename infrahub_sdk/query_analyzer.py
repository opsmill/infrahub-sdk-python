...
# Type alias for schema types returned by the SDK
MainSchemaTypesAPI = NodeSchemaAPI | GenericSchemaAPI | ProfileSchemaAPI | TemplateSchemaAPI


@runtime_checkable
class SchemaBranchProtocol(Protocol):
    """Protocol defining the minimal interface required from a schema branch."""
    ...
    def get(self, name: str, duplicate: bool = False) -> MainSchemaTypesAPI: ...

    def get_node(self, name: str, duplicate: bool = False) -> NodeSchemaAPI: ...

    def get_generic(self, name: str, duplicate: bool = False) -> GenericSchemaAPI: ...

    def get_profile(self, name: str, duplicate: bool = False) -> ProfileSchemaAPI: ...
...
class ContextType(StrEnum):
    ...
    @classmethod
    def from_relationship_cardinality(cls, cardinality: str) -> ContextType:
        if cardinality in {RelationshipCardinality.MANY, "many"}:
            return cls.EDGE
        return cls.NODE
...
class GraphQLOperationType(StrEnum):
    ...
    @classmethod
    def from_operation(cls, operation: OperationType) -> GraphQLOperationType:
        match operation:
            case OperationType.QUERY:
                return cls.QUERY
            case OperationType.MUTATION:
...
class GraphQLQueryModel:
    model: MainSchemaTypesAPI
    root: bool
    arguments: list[GraphQLArgumentInfo]
    attributes: set[str]
...
class GraphQLQueryNode:
    path: str
    operation: GraphQLOperationType = field(default=GraphQLOperationType.UNDEFINED)
    arguments: list[GraphQLArgumentInfo] = field(default_factory=list)
    variables: list[GraphQLVariableInfo] = field(default_factory=list)
    ...
    infrahub_model: MainSchemaTypesAPI | None = field(default=None)
    infrahub_node_models: list[MainSchemaTypesAPI] = field(default_factory=list)
    ...
    def context_model(self) -> MainSchemaTypesAPI | None:
...
class GraphQLQueryReport:
    ...
    @cached_property
    def variables(self) -> list[GraphQLVariableInfo]:
    ...
    def required_argument(self, argument: GraphQLArgumentInfo) -> bool:
...
class InfrahubQueryAnalyzer(GraphQLQueryAnalyzer):
    """GraphQL query analyzer for Infrahub queries.

    This analyzer extends the base GraphQLQueryAnalyzer to provide Infrahub-specific
    analysis capabilities, including:
    - Identifying which Infrahub models are referenced in the query
    - Tracking which attributes and relationships are accessed
    - Determining if a query targets single or multiple objects
    """
    ...
    def _get_operations(self) -> list[GraphQLQueryNode]:
        ...
        for operation_definition in self._operation_definitions:
            ...
            for field_node in selections.field_nodes:
                schema_model: MainSchemaTypesAPI
                infrahub_node_models: list[MainSchemaTypesAPI] = []
                ...
                operational_node = GraphQLQueryNode(
                    operation=GraphQLOperationType.from_operation(operation=operation_definition.operation),
            ...
            if model := query_node.context_model():
                if node.name.value in model.attribute_names or node.name.value == "display_label":
                    ...
                    if rel:
                        ...
                        if isinstance(infrahub_model, GenericSchemaAPI):
                            infrahub_node_models = [
    ...
    def _get_variables(self, operation: OperationDefinitionNode) -> list[GraphQLVariableInfo]:
    ...
    def _parse_arguments(self, field_node: FieldNode) -> list[GraphQLArgumentInfo]:
        return [
            GraphQLArgumentInfo(
