...
async def _aseekable_upload(file_content: BinaryIO | None) -> AsyncIterator[BinaryIO | None]:
    ...
    for the other tasks sharing it. A thread cannot be interrupted: a cancellation arriving during the
    copy is honoured at once, and the worker keeps the temporary file until its pending ``read`` returns,
    then closes it, so the file is never closed under a worker still writing to it.

    Yields:
        BinaryIO | None: ``file_content`` itself when it can be rewound, otherwise its seekable copy.

    Raises:
        asyncio.CancelledError: If the caller is cancelled during the copy.
    ...
    buffer = tempfile.TemporaryFile()  # noqa: SIM115  # the worker closes it if the caller is cancelled mid-copy

    def copy() -> None:
        shutil.copyfileobj(source, buffer)

    def close_after_copy(task: asyncio.Future[None]) -> None:
        if not task.cancelled():
            task.exception()  # nobody is waiting any more; mark a copy error retrieved so asyncio does not log it
        buffer.close()
    ...
    try:
        ...
        if copy_task.done():
            ...
            copy_task.add_done_callback(close_after_copy)
        raise
    try:
        buffer.seek(0)
        # TemporaryFile is typed as IO[bytes]; it offers every call the multipart sender makes.
...
class InfrahubClient(BaseClient):
        ...
        while True:
            ...
            if "errors" in response:
                ...
                if self._retry_handler.is_transient_graphql_errors(errors) and self._retry_handler.should_retry(
                    ...
                    continue
                raise graphql_error_from_response(errors=errors, query=query, variables=variables)
    ...
    async def _get_streaming(
        ...
        Only the stream initiation is retried here: a transient failure while the caller reads the body is
        raised as ``ServerNotResponsiveError``. Pass ``retry_state`` from an outer loop that restarts the whole
        transfer, such as the file handler's download to disk, so both share one retry budget.
        ...
        Raises:
            ServerNotReachableError: If we are not able to connect to the server.
            ServerNotResponsiveError: If the server didn't respond before the timeout expired, or the
                connection was lost while the caller was reading the body.
        ...
        async with httpx.AsyncClient(**self._build_proxy_config(), verify=self.config.tls_context) as client:
            ...
            retry_state = retry_state or self._retry_handler.new_state()
            ...
            try:
                ...
                # Stream initiation maps its own failures inside send(); this is the body read breaking off.
                raise ServerNotResponsiveError(
                    url=url, message=f"Connection to '{url}' was lost while reading the response."
                ) from exc
            except httpx.TimeoutException as exc:
                raise ServerNotResponsiveError(url=url, timeout=request_timeout) from exc
