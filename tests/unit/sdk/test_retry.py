...
from infrahub_sdk.file_handler import FileHandler, FileHandlerSync
...
# --- Lost connections and timeouts, whichever way httpx reports them --------------------------------
...
LOST_CONNECTION_CASES = [
    TransportErrorCase(name="refused", exception=httpx.ConnectError("connection refused")),
    TransportErrorCase(name="handshake-timed-out", exception=httpx.ConnectTimeout("timed out")),
    TransportErrorCase(name="reset", exception=httpx.ReadError("connection reset by peer")),
    TransportErrorCase(
...
TIMEOUT_CASES = [
    TransportErrorCase(name="read", exception=httpx.ReadTimeout("timed out")),
    TransportErrorCase(name="write", exception=httpx.WriteTimeout("timed out")),
    TransportErrorCase(name="pool", exception=httpx.PoolTimeout("timed out")),
]
...
async def test_lost_connection_is_transient_whichever_httpx_error_reports_it(
    client_type: str, case: TransportErrorCase, httpx_mock: HTTPXMock, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    only reaches the retry handler if the client maps it too. ``ConnectTimeout`` is a handshake left
    hanging by a load balancer dropping packets mid-failover, which httpx files under timeouts.
    """
    _no_jitter(monkeypatch)
    recorded_sleeps = _patch_sleep(monkeypatch)
...
async def test_lost_connection_surfaces_as_unreachable_when_retries_are_disabled(
    client_type: str, case: TransportErrorCase, httpx_mock: HTTPXMock
) -> None:
    httpx_mock.add_exception(case.exception, url=GRAPHQL_URL)
...
@pytest.mark.parametrize("client_type", CLIENT_TYPES)
@pytest.mark.parametrize("case", [pytest.param(tc, id=tc.name) for tc in TIMEOUT_CASES])
async def test_timeout_is_transient_whichever_phase_of_the_request_it_hits(
    client_type: str, case: TransportErrorCase, httpx_mock: HTTPXMock, monkeypatch: pytest.MonkeyPatch
) -> None:
    """Every httpx timeout is transient, not only ``ReadTimeout``.

    A write timeout is an upload the server stopped draining; a pool timeout is a connection that
    never became available. Both leave the request unprocessed, so retrying it is safe.
    """
    _no_jitter(monkeypatch)
    recorded_sleeps = _patch_sleep(monkeypatch)
    httpx_mock.add_exception(case.exception, url=GRAPHQL_URL)
...
@pytest.mark.parametrize("client_type", CLIENT_TYPES)
@pytest.mark.parametrize("case", [pytest.param(tc, id=tc.name) for tc in TIMEOUT_CASES])
async def test_timeout_surfaces_as_not_responsive_when_retries_are_disabled(
    client_type: str, case: TransportErrorCase, httpx_mock: HTTPXMock
) -> None:
    httpx_mock.add_exception(case.exception, url=GRAPHQL_URL)
    client = _build_client_over_httpx(client_type, retry_on_failure=False)
...
@pytest.mark.parametrize(
    ("field", "value", "message"),
    [
        pytest.param("retry_delay", 0, "greater than 0", id="retry_delay-zero"),
        pytest.param("retry_max_delay", 0, "greater than 0", id="retry_max_delay-zero"),
        pytest.param("max_retry_duration", -1, "greater than or equal to 0", id="max_retry_duration-negative"),
    ],
)
def test_config_rejects_retry_settings_that_would_spin_or_run_backwards(field: str, value: int, message: str) -> None:
    """A zero base delay or ceiling would make every retry sleep 0s; only the duration may be 0 (unlimited)."""
...
DOWNLOAD_URL = "http://mock/api/storage/files/file-1?branch=main"
DOWNLOAD_BODY = b"streamed body that must be complete on disk\n" * 64


def _truncated_body() -> IteratorStream:
    """A body whose connection drops after the first chunk, the way httpx reports a truncated transfer."""

    def chunks() -> Iterator[bytes]:
        yield DOWNLOAD_BODY[:100]
        raise httpx.RemoteProtocolError("peer closed connection without sending complete message body")

    return IteratorStream(chunks())


async def _download_to(client: InfrahubClient | InfrahubClientSync, dest: Path) -> int:
    if isinstance(client, InfrahubClient):
        return await FileHandler(client=client).download(node_id="file-1", branch="main", dest=dest)
    return FileHandlerSync(client=client).download(node_id="file-1", branch="main", dest=dest)


@pytest.mark.parametrize("client_type", CLIENT_TYPES)
async def test_download_to_file_restarts_when_the_connection_drops_mid_body(
    client_type: str, httpx_mock: HTTPXMock, monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    """Losing the connection while the body streams is transient too: the download starts over on the shared budget."""
    _no_jitter(monkeypatch)
    recorded_sleeps = _patch_sleep(monkeypatch)
    httpx_mock.add_response(url=DOWNLOAD_URL, stream=_truncated_body())
    httpx_mock.add_response(url=DOWNLOAD_URL, content=DOWNLOAD_BODY)
    client = _build_client_over_httpx(client_type, retry_on_failure=True)
    dest = tmp_path / "download.bin"

    assert await _download_to(client, dest) == len(DOWNLOAD_BODY)

    assert dest.read_bytes() == DOWNLOAD_BODY
    assert len(httpx_mock.get_requests()) == 2
    assert recorded_sleeps == [5]


@pytest.mark.parametrize("client_type", CLIENT_TYPES)
async def test_download_interrupted_mid_body_reports_the_lost_connection_and_leaves_no_partial_file(
    client_type: str, httpx_mock: HTTPXMock, tmp_path: Path
) -> None:
    """Without retries the caller learns the transfer broke off, not that the server was unreachable."""
    httpx_mock.add_response(url=DOWNLOAD_URL, stream=_truncated_body())
    client = _build_client_over_httpx(client_type, retry_on_failure=False)
    dest = tmp_path / "download.bin"

    with pytest.raises(ServerNotResponsiveError, match="was lost while reading the response"):
        await _download_to(client, dest)

    assert not dest.exists()
    assert len(httpx_mock.get_requests()) == 1


def _refuse_path_operation(monkeypatch: pytest.MonkeyPatch, name: str, path: Path) -> None:
    """Make ``Path.<name>`` raise ``PermissionError`` for ``path`` only, as a read-only file or directory would.

    Mode bits cannot be used for this: the suite may run as root, which bypasses them. anyio's ``Path``
    delegates to ``pathlib`` in a worker thread, so patching ``pathlib.Path`` covers both clients. ``open``
    is refused for writing only, so the test can still read the file back.
    """
    real = getattr(Path, name)

    def refuse(self: Path, *args: object, **kwargs: object) -> object:
        mode = str(args[0] if args else kwargs.get("mode", "r"))
        if self == path and (name != "open" or "w" in mode):
...
@pytest.mark.parametrize("client_type", CLIENT_TYPES)
async def test_download_leaves_a_dest_it_could_not_open_untouched(
    client_type: str, httpx_mock: HTTPXMock, monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    """A failed open is not a failed transfer: the existing file survives instead of being removed as a partial body."""
    httpx_mock.add_response(url=DOWNLOAD_URL, content=DOWNLOAD_BODY)
    client = _build_client_over_httpx(client_type, retry_on_failure=False)
    dest = tmp_path / "download.bin"
    dest.write_bytes(b"previous content")
    _refuse_path_operation(monkeypatch, "open", dest)
...
@pytest.mark.parametrize("client_type", CLIENT_TYPES)
async def test_download_reports_the_transfer_error_when_removing_the_partial_file_fails(
    client_type: str, httpx_mock: HTTPXMock, monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    """A cleanup that fails must not replace the error explaining why the download stopped."""
    httpx_mock.add_response(url=DOWNLOAD_URL, stream=_truncated_body())
    client = _build_client_over_httpx(client_type, retry_on_failure=False)
    dest = tmp_path / "download.bin"
    _refuse_path_operation(monkeypatch, "unlink", dest)

    with pytest.raises(ServerNotResponsiveError, match="was lost while reading the response"):
        await _download_to(client, dest)

    assert dest.exists(), "the partial file stays when it cannot be removed"
...
async def test_cancelled_async_upload_returns_at_once_and_leaves_the_buffer_to_the_worker(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Cancelling mid-copy surfaces immediately and leaves the temporary file to the worker.

    The worker cannot be interrupted, so it closes the file itself once its pending read returns;
    the file is never closed under it.
    """
    ...
    def recording_temporary_file() -> IO[bytes]:
        buffer = real_temporary_file()
    ...
    assert task in done, "the cancellation did not surface while the worker thread was still blocked"
    with pytest.raises(asyncio.CancelledError):
        await task
    [buffer] = buffers
    assert not buffer.closed, "the temporary file was closed under the worker still copying into it"
    ...
    assert buffer.closed, "the worker did not close the temporary file once its copy ended"
    assert reads[-1] == b"", "the copy should have drained the stream to EOF before closing the buffer"
