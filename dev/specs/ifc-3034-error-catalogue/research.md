...
relogin helper for the same reason. Not `utils.decode_json`: its one addition over `.json()` is raising
`JsonDecodeError`, which a factory built to absorb that case would only catch and discard — and since
